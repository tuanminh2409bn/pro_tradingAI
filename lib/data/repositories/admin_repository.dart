...
class AdminRepository {
  ...
  Stream<SystemStats> getSystemStats() {
    return _firestore.collection('admin').doc('stats').snapshots().map((s) {
      final data = s.data();
      if (data == null) return const SystemStats(dau: 0, mau: 0, growth: 0.0, latency: 0, pendingAlerts: 0);
      return SystemStats(
  ...
  Stream<List<PendingRequest>> getPendingRequests() {
    return _firestore
        .collection('admin').doc('requests').collection('pending')
  ...
  Future<void> approveRequest(String requestId) async {
    await _firestore.collection('admin').doc('requests').collection('pending').doc(requestId).update(
  ...
  Future<AIConfig?> getAIConfig() async {
    final doc = await _firestore.collection('AdminSettings').doc('ai_config').get();
  ...
  Stream<GlobalRiskConfig> getGlobalRisk() {
    return _firestore.collection('admin').doc('global_risk').snapshots().map((s) {
  ...
  Future<void> saveGlobalRisk(GlobalRiskConfig config) async {
    await _firestore.collection('admin').doc('global_risk').set(config.toMap(), SetOptions(merge: true));
  }
  ...
  Stream<bool> getKillSwitchState() {
    return _firestore.collection('admin').doc('system_config').snapshots().map((s) {
  ...
  Stream<List<ServiceStatus>> getServiceStatus() {
    return _firestore.collection('admin').doc('service_status').snapshots().map((s) {
      final data = s.data();
      if (data == null) return _defaultServiceStatus();
      return [
        ServiceStatus(name: 'MT4 Bridge', isOnline: data['mt4_online'] ?? false, latencyMs: (data['mt4_latency'] ?? 0).toInt()),
        ServiceStatus(name: 'AI Analyzer', isOnline: data['ai_online'] ?? false, latencyMs: (data['ai_latency'] ?? 0).toInt()),
        ServiceStatus(name: 'Data Feeder', isOnline: data['data_online'] ?? false, latencyMs: (data['data_latency'] ?? 0).toInt()),
      ];
    });
  }
  ...
  Future<List<Map<String, dynamic>>> getDailyStats(int days) async {
    ...
    try {
      final snap = await _firestore.collection('admin').doc('daily_stats').collection('days')
          .orderBy(FieldPath.documentId, descending: true).limit(days).get();
